...
## Bridge fix — [footnote] shortcode + bootstrap bugs (2026-04-19)

- **Problema reportado:** usuario con `[footnote]...[/footnote]` en post; `/p/:slug` devolvía tags literales sin renderizar. **Date:** 2026-04-19
- **Causa raíz #1 (pilot incorrecto):** piloto `footnotes.ts` sólo procesaba la sintaxis MCI `((texto))` y no registraba `add_shortcode('footnote', ...)`. La memoria 2026-04-19 PoC ya documentaba el escenario "WP shortcode real" — la implementación del piloto se quedó en la sintaxis legacy. **Date:** 2026-04-19
- **Causa raíz #2 (bug crítico pre-existente en bootstrap):** `buildBootstrapCode` en `packages/server/src/bridge/index.ts` tenía `shortcode_parse_atts` con un regex PHP que se escribió en un **template literal JS con comillas `\'`**. JS interpreta `\'` dentro de backticks como un apóstrofo literal (el backslash se consume), dejando la cadena PHP single-quoted con apóstrofos sin escapar → PHP parse error "unexpected token ^ expecting ]" en la línea 227. Fix: `\'` → `\\'` en las dos regex afectadas. **Date:** 2026-04-19
- **Causa raíz #3 (bug crítico pre-existente en bootstrap):** 11 funciones PHP built-in (`exec`, `system`, `shell_exec`, `passthru`, `popen`, `proc_open`, `mail`, `curl_exec`, `curl_multi_exec`, `file_put_contents`, `fwrite`) se redeclaraban en cada run. `php-wasm` stdlib ya las provee → fatal `Cannot redeclare exec() in /internal/eval.php on line 21` siempre. Fix: envolver cada una con `if (!function_exists(...))`. **Date:** 2026-04-19
- **Impacto histórico:** con `NODEPRESS_TIER2=true` en producción, **ningún shortcode jamás funcionó via bridge PHP real**. El fail-safe `BRIDGE_FATAL → passthrough` enmascaró el bug durante sprints. Los 241 tests "verdes" del Sprint 2 no lo detectaron porque `bridge.integration.test.ts` mockea `@php-wasm/node` y `@php-wasm/universal`. El único test que ejecuta php-wasm real vive en `packages/spike-phpwasm`, pero no ejercitaba `[footnote]`. Este es exactamente el patrón que la lección R-2 de 2026-04-19 (su_spacer) identificó: tests JS simulados producen falsos verdes mientras el PHP real está roto. **Date:** 2026-04-19
- **Fix final (3 archivos):** (1) `bridge/index.ts` — escape `\\'` en regex + 11 guards `function_exists`; (2) `bridge/pilots/footnotes.ts` — `add_shortcode('footnote', ...)` que apila en `$GLOBALS['__np_footnotes']` y devuelve `<sup>` anchor, pre-pass legacy `((text))` compartiendo contador, `do_shortcode` en el pilot antes del apéndice `<div class="footnotes">`; (3) test suite del piloto actualizada (14/14 verde). **Date:** 2026-04-19
- **Trade-off documentado doble `do_shortcode`:** el pilot llama `$postContent = do_shortcode($postContent);` para poder appendear el section; el runner del bridge vuelve a llamar `do_shortcode` en el wrap final. Segunda llamada es no-op porque los tags ya están consumidos. Coste: microsegundos PHP-WASM. Beneficio: cero cambios al contrato del runner, cero ADR amendment, pilot autocontenido. **Date:** 2026-04-19
- **Lección arquitectónica crítica R-4:** mocks de `@php-wasm/*` en tests de integración del bridge son un **anti-patrón** cuando el bug vive en el código PHP generado (escape de strings, redeclaración de built-ins). Único test fiable = php-wasm real ejecutando el runner completo. El spike `packages/spike-phpwasm` debería tener regression tests por piloto. Ticket bloqueante Sprint 7+ para Raúl. **Date:** 2026-04-19
- **Lección arquitectónica R-5:** cualquier string PHP escrito dentro de un **template literal JS** debe usar `\\'` (dos backslashes) para un apóstrofo literal. El intuitivo `\'` lo consume JS. Pattern reusable: generar PHP desde TS requiere tests que **comparen el output string** con un fixture, no solo que compilen. Aplicar a todos los pilots que usen regex con comillas. **Date:** 2026-04-19
- **Lección operacional R-6:** BRIDGE_FATAL con `passthrough html = input` es fail-safe correcto para producción, pero **oculta bugs del bootstrap** porque el usuario ve contenido "normal" en vez de error. El span emite `warnings_count: 1` pero NO el warning completo. Propuesta para Sprint 7: añadir `error_detail_hash` al span (primeros 64 bytes del warning hasheados) para dashboards de observability → detectar patterns de fallo recurrentes sin leakear contenido. **Date:** 2026-04-19
- **Verificación curl end-to-end:** `curl http://localhost:3000/p/footnotes-demo-1776645605611` devuelve `<sup id="fnref-1"><a href="#fn-1" class="footnote-ref">1</a></sup>` + `<div class="footnotes"><ol><li id="fn-1">...<a href="#fnref-1" class="footnote-backref">&#8617;</a></li></ol></div>`. Dos shortcodes en el post → dos anchors numerados + dos entries. **Date:** 2026-04-19

## Demo video — PHP plugins multi-shortcode ampliación (2026-04-19)

- **Demo spec `admin/e2e/demo/demo-php-plugin.spec.ts` ampliado de 1→3 pilotos** tras el fix del bridge Footnotes (commit anterior). El spec previo mostraba sólo `[footnote]` sin asserts (herencia del período BRIDGE_FATAL). Ahora: seeds 3 plugins (Footnotes, Shortcodes Ultimate, Contact Form 7) + post con shortcodes mezclados + asserts sobre HTML renderizado. **Date:** 2026-04-19
- **Pilots incluidos y rationale:**
  - Footnotes: shortcode `[footnote]` → `<sup id="fnref-N">` + `<div class="footnotes">`. Impresionante + determinista. Asserted.
  - Shortcodes Ultimate: `[su_note]` → `<div class="su-note">`, `[su_button url=… color=blue]` → `<a class="su-button su-button-blue">` con `href` asserted. Output visual (caja, botón) — gran beat de video.
  - Contact Form 7: `[contact-form-7 id=1]` → `<div class="wpcf7"><form class="wpcf7-form">` con campos name/email/message. Form completo en pantalla = visualmente potente. Helena lo excluyó del PoC producto; para un demo interno es legítimo porque sólo renderizamos HTML (no hay submit).
- **Display Posts EXCLUIDO deliberadamente:** el REST handler (`serialize.ts:toWpPostAsync`) NO pasa `context.candidatePosts` al `renderShortcodes()`. El shortcode `[display-posts]` itera `$np_candidate_posts` que llega como `[]` → devuelve `''`. Output vacío = mal beat de video. Revisitable cuando el handler alimente candidatos. **Date:** 2026-04-19
- **Asserts reales en scene 6** (antes sólo `waitForTimeout(5000)`): `page.locator("article")` como contenedor + sub-locators por piloto. Cada familia tiene marcadores únicos (fnref-N, su-button-blue, wpcf7-form) que hacen el test robusto si un piloto regresara. **Date:** 2026-04-19
- **Regla confirmada R-2 (memoria previa):** el demo-video NO es script de un solo uso — es artefacto de producto (ADR-016 Demo Lifecycle Contract). Ampliarlo con asserts sobre HTML real convierte el video en un test de regresión visual del bridge Tier 2 completo, no sólo en una grabación. Sin los asserts, un pilot podría regresar silenciosamente a `BRIDGE_FATAL → passthrough` y el video generado seguiría "verde". **Date:** 2026-04-19
- **Docstring del spec actualizada:** removida nota outdated de `BRIDGE_FATAL`, añadida tabla explícita de pilots incluidos + exclusión documentada de `[display-posts]` con su precondición técnica. Evita que alguien Sprint N+3 añada `[display-posts]` al post sin revisar el serialize. **Date:** 2026-04-19
- **Ejecución `npm run demo:video:php`:** 1/1 verde en 25.2s contra backend real con `NODEPRESS_TIER2=true`. Video ~958KB en `admin/test-results/demo-php-plugin-NodePress--029ca-hortcode-post-→-public-view-chromium-demo/video.webm`. **Date:** 2026-04-19
- **Lección arquitectónica consolidada (ya en tech_memory conceptualmente):** cuando se amplía un demo existente, usar los mismos pilots que el backend ya inyecta estáticamente (ACTIVE_PILOTS) NO abre superficie de ataque — son compile-time commitments. Añadir pilots al demo no requiere ADR amendment; eliminar uno sí podría si rompe la paridad demo↔prod. **Date:** 2026-04-19
